...
// Renamed prop for clarity: isMobileOpen indicates if the mobile overlay sidebar is open
const Sidebar = ({ isMobileOpen, onMobileClose }) => { 
  ...
  const getNavLinkClasses = (isActive) => `${navCardBase} ${isActive ? 'bg-nuzum-accent-primary text-nuzum-accent-primary-content shadow-md scale-105' : 'bg-nuzum-surface text-nuzum-text-secondary border border-nuzum-border shadow-sm hover:bg-nuzum-border hover:text-nuzum-text-primary hover:-translate-y-px active:scale-98'}`;
  ...
  const NavItem = ({ to, icon: Icon, labelKey }) => (
    <NavLink 
      to={to} 
      onClick={onMobileClose} // Closes mobile overlay when a nav item is clicked
  ...
  return (
    <>
      ...
      <aside 
        className={`
          w-60 bg-nuzum-sidebar-bg text-nuzum-text-primary flex-shrink-0 flex flex-col shadow-xl border-e border-nuzum-border
          transform transition-transform duration-300 ease-in-out
          
          md:w-64 md:sticky md:top-0 md:h-screen md:translate-x-0 {/* Desktop: sticky, always visible in flow */}
          
          fixed inset-y-0 start-0 z-40 {/* Mobile: fixed overlay, highest z-index */}
          ${i18n.dir() === 'rtl' ? 
            (isMobileOpen ? 'translate-x-0' : 'translate-x-full') :  // RTL: slides from right for mobile
            (isMobileOpen ? 'translate-x-0' : '-translate-x-full')    // LTR: slides from left for mobile
          }
          ${!isMobileOpen ? 'md:translate-x-0' : ''} {/* Ensures desktop is not affected by mobile's translate-x-full */}
        `}
      >
