...
// Configure Decimal.js precision (optional, default is usually enough for currency)
...
async function updateRelatedData(newTransactionDetails, oldTransactionDetails = null) {
  ...
  if (accountIdToUpdate) {
      const accountUpdatePromise = supabase.from('accounts').select('balance').eq('id', accountIdToUpdate).single();
      updates.push(accountUpdatePromise.then(async ({ data: account, error }) => {
        if (error || !account) { return Promise.reject(new Error('Failed to fetch account for balance update.')); }
        
        let currentBalance = new Decimal(account.balance);
        let calculatedNewBalance = currentBalance; 

        if (isEdit || isDelete) {
          const oldAmount = new Decimal(oldTransactionDetails.amount);
          if (oldTransactionDetails.type === 'income') calculatedNewBalance = currentBalance.minus(oldAmount);
          else calculatedNewBalance = currentBalance.plus(oldAmount);
          // Update currentBalance for the next step if it's an edit
          if(isEdit) currentBalance = calculatedNewBalance; 
        }

        if (isCreate || isEdit) {
          const newAmount = new Decimal(newTransactionDetails.amount);
          if (newTransactionDetails.type === 'income') calculatedNewBalance = currentBalance.plus(newAmount);
          else calculatedNewBalance = currentBalance.minus(newAmount);
        }
        
        // Compare original currentBalance with final calculatedNewBalance
        const originalCurrentBalanceForComparison = new Decimal(account.balance);
        if (!originalCurrentBalanceForComparison.equals(calculatedNewBalance) || isCreate || isDelete) {
          const { error: updateBalanceError } = await supabase.from('accounts').update({ balance: calculatedNewBalance.toFixed(2) }).eq('id', accountIdToUpdate);
          if (updateBalanceError) { return Promise.reject(new Error('Failed to update account balance.')); }
        }
      }));
  }
  ...
  if (materialIdToUpdate) {
    const materialUpdatePromise = supabase.from('materials').select('quantity').eq('id', materialIdToUpdate).single();
    updates.push(materialUpdatePromise.then(async ({ data: material, error }) => {
      if (error || !material) { return Promise.reject(new Error('Failed to fetch material for quantity update.')); }
      let currentQuantity = new Decimal(material.quantity);
      ...
      if ((isEdit || isDelete) && oldTransactionDetails.material_id === materialIdToUpdate && oldTransactionDetails.material_quantity_affected !== undefined && oldTransactionDetails.material_quantity_affected !== null) {
        calculatedNewQuantity = currentQuantity.minus(new Decimal(oldTransactionDetails.material_quantity_affected));
        if(isEdit) currentQuantity = calculatedNewQuantity;
      }
      if ((isCreate || isEdit) && newTransactionDetails.material_id === materialIdToUpdate && newTransactionDetails.material_quantity_affected !== undefined && newTransactionDetails.material_quantity_affected !== null) {
        calculatedNewQuantity = currentQuantity.plus(new Decimal(newTransactionDetails.material_quantity_affected));
      }
      
      const originalCurrentQuantityForComparison = new Decimal(material.quantity);
      if (!originalCurrentQuantityForComparison.equals(calculatedNewQuantity) || isCreate || isDelete) {
        const { error: updateQtyError } = await supabase.from('materials').update({ quantity: calculatedNewQuantity.toNumber() }).eq('id', materialIdToUpdate); // Store as number
        if (updateQtyError) { return Promise.reject(new Error('Failed to update material quantity.')); }
      }
    }));
  }
  try { await Promise.all(updates); } 
  catch (e) { console.error("updateRelatedData: Error in Promise.all:", e); toast.error(t('errorUpdatingRelatedData')); throw e; }
}

// ... THE REST OF TransactionsPage.jsx REMAINS EXACTLY THE SAME AS THE LAST FULL VERSION I PROVIDED ...
// (Make sure to copy it from the message that starts with "You are absolutely right, and I sincerely apologize for this repeated error...")
// The important part is only the `updateRelatedData` function and the `Decimal` import at the top.
const TransactionsPage = () => {
  ...
  const [searchTerm, setSearchTerm] = useState(''); const [filterType, setFilterType] = useState(''); const [filterCurrency, setFilterCurrency] = useState(''); const [filterAccountId, setFilterAccountId] = useState(''); const [filterMaterialId, setFilterMaterialId] = useState(''); const [filterDateFrom, setFilterDateFrom] = useState(''); const [filterDateTo, setFilterDateTo] = useState('');
  const [accounts, setAccounts] = useState([]);
  const [materials, setMaterials] = useState([]);
  const [availableCurrencies, setAvailableCurrencies] = useState([]);
  const [initialDataLoading, setInitialDataLoading] = useState(true);

  const loadDropdownData = useCallback(async () => { if (!user) { setAccounts([]); setMaterials([]); setAvailableCurrencies([]); setInitialDataLoading(false); return; } setInitialDataLoading(true); try { const [accRes, matRes, currRes] = await Promise.all([ supabase.from('accounts').select('id, name, currency, balance').eq('user_id', user.id).order('name', { ascending: true }), supabase.from('materials').select('id, name, unit_type').eq('user_id', user.id).order('name', { ascending: true }), getAllCurrenciesForUser(user.id) ]); if (accRes.error) throw accRes.error; setAccounts(accRes.data || []); if (matRes.error) throw matRes.error; setMaterials(matRes.data || []); setAvailableCurrencies(currRes || []); } catch (err) { console.error("TransactionsPage: Error fetching initial page/dropdown data:", err.message); toast.error(t('errorFetchingDropdownData')); setAccounts([]); setMaterials([]); setAvailableCurrencies([]); } finally { setInitialDataLoading(false); }}, [user, t]);
  useEffect(() => { loadDropdownData(); }, [loadDropdownData]);
  const fetchTransactions = useCallback(async () => { if (!user || initialDataLoading) { if(!user) setPageLoading(false); return; } setPageLoading(true); setPageError(''); try { let q = supabase.from('transactions').select(`id, created_at, user_id, type, amount, currency, date, notes, account_id, material_id, material_quantity_affected, image_url, accounts:account_id ( id, name, currency ), materials ( id, name, unit_type )`).eq('user_id', user.id); if (searchTerm) q = q.ilike('notes', `%${searchTerm}%`); if (filterType) q = q.eq('type', filterType); if (filterCurrency) q = q.eq('currency', filterCurrency); if (filterAccountId) q = q.eq('account_id', filterAccountId); if (filterMaterialId) q = q.eq('material_id', filterMaterialId); if (filterDateFrom) q = q.gte('date', filterDateFrom); if (filterDateTo) { const tD = new Date(filterDateTo); tD.setHours(23,59,59,999); q = q.lte('date', tD.toISOString()); } q = q.order('date', { ascending: false }).order('created_at', { ascending: false }); const { data, error: fe } = await q; if (fe) { toast.error(t('errorFetchingTransactions') + `: ${fe.message}`); setTransactions([]); throw fe; } setTransactions(data || []); } catch (err) { setPageError(t('errorFetchingTransactions')); setTransactions([]); } finally { setPageLoading(false); }}, [user, t, searchTerm, filterType, filterCurrency, filterAccountId, filterMaterialId, filterDateFrom, filterDateTo, initialDataLoading]);
  useEffect(() => { if(!initialDataLoading) fetchTransactions(); }, [fetchTransactions, initialDataLoading]);
  useEffect(() => { if (!user) return; const ch = supabase.channel('public:transactions_page_decimal_fix').on('postgres_changes', { event: '*', schema: 'public', table: 'transactions', filter: `user_id=eq.${user.id}` }, () => fetchTransactions()).on('postgres_changes', { event: '*', schema: 'public', table: 'accounts', filter: `user_id=eq.${user.id}`}, () => loadDropdownData()).on('postgres_changes', { event: '*', schema: 'public', table: 'materials', filter: `user_id=eq.${user.id}`}, () => loadDropdownData()).subscribe(); return () => supabase.removeChannel(ch); }, [user, fetchTransactions, loadDropdownData]);
  ...
  const exportToCSV = () => { /* ... same ... */}; const exportToPDF = async () => { /* ... same ... */};
