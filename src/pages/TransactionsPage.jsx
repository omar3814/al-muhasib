...
async function updateRelatedData(newTransactionDetails, oldTransactionDetails = null) { /* ... (This function should be the robust version from previous fixes) ... */ const updates = []; const isCreate = !oldTransactionDetails && newTransactionDetails && newTransactionDetails.type; const isDelete = !!oldTransactionDetails && (!newTransactionDetails || !newTransactionDetails.type || (Object.keys(newTransactionDetails).length <= 2 && newTransactionDetails.account_id && newTransactionDetails.material_id !== undefined)); const isEdit = !!oldTransactionDetails && !!newTransactionDetails && newTransactionDetails.type; const accountIdToUpdate = oldTransactionDetails?.account_id || newTransactionDetails?.account_id; if (accountIdToUpdate) { const accountUpdatePromise = supabase.from('accounts').select('balance').eq('id', accountIdToUpdate).single(); updates.push(accountUpdatePromise.then(async ({ data: account, error }) => { if (error || !account) { return Promise.reject(new Error('Failed to fetch account for balance update.')); } let currentBalance = parseFloat(account.balance); let newBalance = currentBalance; if (isEdit || isDelete) { if (oldTransactionDetails.type === 'income') newBalance -= parseFloat(oldTransactionDetails.amount); else newBalance += parseFloat(oldTransactionDetails.amount); } if (isCreate || isEdit) { if (newTransactionDetails.type === 'income') newBalance += parseFloat(newTransactionDetails.amount); else newBalance -= parseFloat(newTransactionDetails.amount); } if (Math.abs(currentBalance - newBalance) > 0.0001 || isCreate || isDelete) { const { error: updateBalanceError } = await supabase.from('accounts').update({ balance: newBalance.toFixed(2) }).eq('id', accountIdToUpdate); if (updateBalanceError) { return Promise.reject(new Error('Failed to update account balance.')); } } })); } const materialIdToUpdate = oldTransactionDetails?.material_id || newTransactionDetails?.material_id; if (materialIdToUpdate) { const materialUpdatePromise = supabase.from('materials').select('quantity').eq('id', materialIdToUpdate).single(); updates.push(materialUpdatePromise.then(async ({ data: material, error }) => { if (error || !material) { return Promise.reject(new Error('Failed to fetch material for quantity update.')); } let currentQuantity = parseInt(material.quantity); let newQuantity = currentQuantity; if ((isEdit || isDelete) && oldTransactionDetails.material_id === materialIdToUpdate && oldTransactionDetails.material_quantity_affected !== undefined && oldTransactionDetails.material_quantity_affected !== null) { newQuantity -= parseInt(oldTransactionDetails.material_quantity_affected); } if ((isCreate || isEdit) && newTransactionDetails.material_id === materialIdToUpdate && newTransactionDetails.material_quantity_affected !== undefined && newTransactionDetails.material_quantity_affected !== null) { newQuantity += parseInt(newTransactionDetails.material_quantity_affected); } if (currentQuantity !== newQuantity || isCreate || isDelete) { const { error: updateQtyError } = await supabase.from('materials').update({ quantity: newQuantity }).eq('id', materialIdToUpdate); if (updateQtyError) { return Promise.reject(new Error('Failed to update material quantity.')); } } })); } try { await Promise.all(updates); } catch (e) { toast.error(t('errorUpdatingRelatedData')); throw e; }}
...
const TransactionsPage = () => {
  ...
  useEffect(() => { if (!user) return; const ch = supabase.channel('public:transactions_page_pdf_fix_v2').on('postgres_changes', { event: '*', schema: 'public', table: 'transactions', filter: `user_id=eq.${user.id}` }, () => fetchTransactions()).on('postgres_changes', { event: '*', schema: 'public', table: 'accounts', filter: `user_id=eq.${user.id}`}, () => loadDropdownData()).on('postgres_changes', { event: '*', schema: 'public', table: 'materials', filter: `user_id=eq.${user.id}`}, () => loadDropdownData()).subscribe(); return () => supabase.removeChannel(ch); }, [user, fetchTransactions, loadDropdownData]);
  ...
  const exportToCSV = () => {
    const enTranslations = i18n.getResourceBundle('en', 'common');
    const getEn = (key, fb = '') => enTranslations[key] || fb || key;

    const dataToExport = transactions.map(tx => {
      const txTypeStr = tx.type ? getEn(tx.type.toLowerCase(), tx.type) : getEn('unknown', 'Unknown');
      const unitTypeStr = tx.materials?.unit_type ? getEn(tx.materials.unit_type.toLowerCase(), tx.materials.unit_type) : '';
      
      return {
        [getEn('date', 'Date')]: tx.date ? new Date(tx.date).toLocaleDateString('en-CA') : '-',
        [getEn('type', 'Type')]: txTypeStr,
        [getEn('notes', 'Notes')]: tx.notes || '-',
        [getEn('amount', 'Amount')]: typeof tx.amount === 'number' ? tx.amount.toFixed(2) : '-',
        [getEn('currency', 'Currency')]: tx.currency || '-',
        [getEn('account', 'Account')]: tx.accounts?.name || getEn('unknownAccount', 'Unknown Account'),
        [getEn('material', 'Material')]: tx.materials?.name || '-',
        [getEn('materialQuantity', 'Qty')]: tx.material_id && tx.material_quantity_affected !== null ? Math.abs(tx.material_quantity_affected) : '-',
        [getEn('materialUnit', 'Unit')]: tx.material_id && unitTypeStr ? unitTypeStr : '-',
      };
    });
    const csv = Papa.unparse(dataToExport);
    const blob = new Blob([`\uFEFF${csv}`], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    if (link.download !== undefined) { const url = URL.createObjectURL(blob); link.setAttribute('href', url); link.setAttribute('download', `Nuzum_Transactions_${new Date().toISOString().split('T')[0]}.csv`); link.style.visibility = 'hidden'; document.body.appendChild(link); link.click(); document.body.removeChild(link); }
  };
  ...
  const exportToPDF = async () => {
    ...
    const tableColumns = [
        { header: getEn('date', 'Date'), dataKey: 'dateStr' }, 
        { header: getEn('type', 'Type'), dataKey: 'typeStr' }, 
        { header: getEn('notes', 'Notes'), dataKey: 'notes' }, 
        { header: getEn('account', 'Account'), dataKey: 'accountName' }, 
        // { header: getEn('material', 'Material'), dataKey: 'materialName' }, // Keep it simpler for PDF
        { header: getEn('amount', 'Amount'), dataKey: 'amountStr' }, 
        { header: getEn('currency', 'Currency'), dataKey: 'currencyCode' } // Use code for consistency
    ];
    
    const tableRows = transactions.map(tx => {
      const txTypeStr = tx.type ? getEn(tx.type.toLowerCase(), tx.type) : getEn('unknown', 'Unknown');
      return {
        dateStr: tx.date ? new Date(tx.date).toLocaleDateString('en-CA') : '-',
        typeStr: txTypeStr,
        notes: tx.notes || '-',
        accountName: tx.accounts?.name || getEn('unknownAccount', 'Unknown Account'),
        // materialName: tx.materials?.name || '-', // Removed for simplicity in PDF
        amountStr: typeof tx.amount === 'number' ? tx.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-',
        currencyCode: tx.currency || '-',
      };
    });

    doc.setProperties({ title: getEn('transactionsReport', 'Transactions Report'), subject: getEn('appName', 'Nuzum') + ' - ' + getEn('transactions', 'Transactions'), creator: getEn('appName', 'Nuzum') });
    doc.setFontSize(18); doc.setTextColor(40); doc.text(getEn('transactionsReport', 'Transactions Report'), 14, 20);
    doc.setFontSize(10); doc.setTextColor(100); doc.text(`${getEn('reportGeneratedOn')}: ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}`, 14, 26);

    doc.autoTable({
        columns: tableColumns,
        body: tableRows,
        startY: 35, theme: 'grid', 
        headStyles: { fillColor: [51, 65, 85], textColor: [240, 246, 252], fontStyle: 'bold', fontSize: 9 },
        styles: { font: 'helvetica', fontSize: 8, cellPadding: 2.5, overflow: 'linebreak', valign: 'middle' },
        columnStyles: { 
            notes: { cellWidth: 'auto' }, 
            amountStr: { halign: 'right' }, 
            currencyCode: { halign: 'center', cellWidth: 15 }
        },
        didDrawPage: function (data) { const pageCount = doc.internal.getNumberOfPages(); doc.setFontSize(8); doc.setTextColor(150); doc.text( `Page ${data.pageNumber} of ${pageCount} | ${getEn('appName', 'Nuzum')}`, data.settings.margin.left, doc.internal.pageSize.getHeight() - 10 ); }
    });
    doc.save(`Nuzum_Transactions_${new Date().toISOString().split('T')[0]}.pdf`);
  };
